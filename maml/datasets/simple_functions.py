...
def generate_sinusoid_batch(amp_range,
                            phase_range,
                            input_range,
                            num_samples,
                            batch_size,
  ...
  def __iter__(self):
    for batch in range(self._num_total_batches):
      ...
      for task in range(self._meta_batch_size):
        # print(inputs, task, inputs.shape)
        task_inputs = torch.tensor(inputs[task],
                                   device=self._device,
                                   dtype=self._dtype)
        task_outputs = torch.tensor(outputs[task],
                                    device=self._device,
                                    dtype=self._dtype)
        task_infos = infos[task]
        # print(task_inputs, task_inputs.shape)
        train_task = Task(task_inputs[self._num_val_samples:],
                          task_outputs[self._num_val_samples:], task_infos)
...
class FiveFunctionsMetaDataset(SimpleFunctionDataset):
  ...
  def _generate_batch(self):
    ...
    inputs = np.concatenate(
        (sin_inputs, lin_inputs, qua_inputs, tanh_inputs, abs_inputs))
    outputs = np.concatenate(
        (sin_outputs, lin_outputs, qua_outputs, tanh_outputs, abs_outputs))
